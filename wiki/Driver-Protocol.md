...
| 10 | Added the InputHealth feature pipe `\\.\pipe\OpenVR-WKInputHealth`, `RequestSetInputHealthConfig`, and `RequestResetInputHealthStats`. The driver observes OpenVR boolean/scalar component traffic and publishes diagnostics snapshots through `OpenVRPairInputHealthMemoryV1`; default config is diagnostics-only/pass-through. The `Request` union size is unchanged because both new payloads are smaller than `SetDeviceTransform`; the bump forces paired overlay+driver installs so the new request types fail loudly on stale builds. |
