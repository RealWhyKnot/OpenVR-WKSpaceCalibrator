...
- **Silent drift correction for one-shot users (opt-in).** A passive subsystem watches for natural moments to silently re-fit your calibration: T-poses (VRChat IK), hand-on-HMD adjustment, idle stillness, residual-EMA, HMD wake, floor-touch Y-anchor, and HMD-recenter compensation. **Off by default** -- in current testing it can produce worse tracking than no correction; toggle on via the "Silent drift correction" checkbox if you want to help debug it. See [Continuous Calibration § Silent drift correction](Continuous-Calibration#silent-drift-correction-one-shot-users-only).
...
- **Non-continuous UI parity.** One-shot users get the same tabbed depth (Settings / Advanced / Prediction / Logs) as continuous-mode users -- previous versions hid all of those behind clicking "Continuous Calibration" first. Debug logs, silent-recal opt-in, jitter threshold, lock mode, etc. are all reachable without entering continuous mode.
