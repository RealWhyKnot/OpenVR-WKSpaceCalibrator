...
- **Silent drift correction for one-shot users.** When continuous calibration is off and a profile is loaded, a passive subsystem watches for drift and silently corrects it via several triggers — T-pose detection (perfect for VRChat), hand-on-HMD adjustment, idle-pose stillness, residual-EMA monitoring, HMD wake events, and floor-touch Y-anchor. None of these need user knowledge or action; they fire on natural moments the user produces during normal play. See [Continuous Calibration § Silent drift correction](Continuous-Calibration#silent-drift-correction-one-shot-users-only).
- **HMD recenter compensation.** When the HMD's pose jumps by >30 cm or >30° in one tick (Quest Home-button recenter, Oculus Link reset), the same delta is applied to every stored calibrated transform so body trackers stay aligned with the user instead of flying across the room.
