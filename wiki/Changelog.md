...
All notable changes to OpenVR-SpaceCalibrator (RealWhyKnot fork). The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) and the project uses date-driven versioning (`YYYY.M.D.N` for releases, `YYYY.M.D.N-XXXX` for dev builds -- see [[Building]] for shape rules).
