...
The `.md` files in this folder are the **source of truth** for the GitHub Wiki at <https://github.com/RealWhyKnot/OpenVR-SpaceCalibrator/wiki>. They are mirrored to the wiki repo by [`.github/workflows/wiki-sync.yml`](../../.github/workflows/wiki-sync.yml) on every push to `develop` that touches `docs/wiki/**`.
