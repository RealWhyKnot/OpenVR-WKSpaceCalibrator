...
// magnitude has been above the plausibility bound (200 cm) for at least
...
TEST(WedgeDetectorTest, FirstTickAboveBoundStampsButDoesNotFire) {
    ...
    EXPECT_FALSE(ShouldFireRuntimeWedgeRecovery(/*mag=*/250.0,
...
TEST(WedgeDetectorTest, SustainedAboveBoundWithinDebounceDoesNotFire) {
    ...
    ShouldFireRuntimeWedgeRecovery(250.0, 100.0, wedgeSince);
    ASSERT_DOUBLE_EQ(wedgeSince, 100.0);

    // 5 s later, still above bound: no fire, timer unchanged.
    EXPECT_FALSE(ShouldFireRuntimeWedgeRecovery(260.0, 105.0, wedgeSince));
    EXPECT_DOUBLE_EQ(wedgeSince, 100.0);

    // 29.9 s later: still under the debounce ceiling, no fire.
    EXPECT_FALSE(ShouldFireRuntimeWedgeRecovery(260.0, 129.9, wedgeSince));
    EXPECT_DOUBLE_EQ(wedgeSince, 100.0);
}
...
TEST(WedgeDetectorTest, OverBoundForDebounceWindowFiresOnceThenSilent) {
    ...
    EXPECT_TRUE(ShouldFireRuntimeWedgeRecovery(250.0, fireTime, wedgeSince));
    EXPECT_LT(wedgeSince, 0.0)
    ...
    EXPECT_FALSE(ShouldFireRuntimeWedgeRecovery(250.0, fireTime + 0.1, wedgeSince));
...
TEST(WedgeDetectorTest, TransientSpikeBelowDebounceDoesNotFire) {
    ...
    EXPECT_FALSE(ShouldFireRuntimeWedgeRecovery(250.0, 200.0, wedgeSince));
    EXPECT_DOUBLE_EQ(wedgeSince, 200.0);
}

// ---------------------------------------------------------------------------
// Healthy fresh-start hunting (8-12 cm transients during the first ~10 min
// after a cold-start) is nowhere near the 200 cm threshold, so no firing.
